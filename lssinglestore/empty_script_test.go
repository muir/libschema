...
func TestEmptyScriptNoOp(t *testing.T) {
	...
	if dsn == "" { t.Skip("Set $LIBSCHEMA_SINGLESTORE_TEST_DSN to run SingleStore tests") }
	...
	if m == nil { t.Fatalf("migration not registered") }
	if !m.Base().Status().Done { t.Fatalf("empty script should mark migration done") }
}
