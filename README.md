...
## Statement Classification & Idempotency (Advanced)

Libschema parses SQL scripts to enforce safety and consistent behavior across dialects. A lightweight tokenization + classification layer (internal `stmtclass`) assigns flags to each statement:

Flags (summarized):

- IsDDL: Statement changes schema (CREATE / ALTER / DROP ...)
- IsDML: Statement manipulates data (INSERT / UPDATE / DELETE / SELECT ...)
- IsMultipleStatements: Script contained more than one top-level statement (split on semicolons outside literals)
- IsNonIdempotent: DDL that will fail on re-run without manual cleanup (e.g. CREATE TABLE without IF NOT EXISTS)
- IsEasilyIdempotentFix: A guard can trivially be added (e.g. CREATE TABLE -> CREATE TABLE IF NOT EXISTS). Used to escalate errors where safety is easy to achieve.

Dialects currently supported: MySQL / SingleStore and PostgreSQL. Classification accounts for differences in available guards and common syntactic variants.

### Sentinel Errors

Two primary sentinel errors are exported (string-based so `errors.Is` works across wrapping):

- ErrDataAndDDL: A single migration script mixed DDL and DML in a context where that’s disallowed (e.g. transactional MySQL migrations). Split the script into separate migrations.
- ErrNonIdempotentDDL: A non-transactional migration contains easily-guarded DDL that is not idempotent (e.g. CREATE TABLE lacking IF NOT EXISTS). Add EXISTS guards to make the script safe to retry.

These used to be raised by an internal package named `stmtcheck`; that logic has been replaced by dialect-aware `stmtclass` (the name change is internal only—public semantics are stable).

### Non-Transactional Enforcement

Drivers may (or may be forced to) run a migration outside an explicit transaction when the dialect requires it (e.g. PostgreSQL concurrent index creation). Non-transactional migrations must be idempotent. Libschema enforces:

1. Single statement (multi-statement scripts increase risk of partial application).
2. Easily-guarded DDL must have IF (NOT) EXISTS (otherwise ErrNonIdempotentDDL).
3. Mixed data + DDL patterns are rejected (ErrDataAndDDL) to avoid ambiguous partially-applied data changes.

### Upgrading From Older Versions (AnalyzeTokens / stmtcheck)

Earlier versions referenced an internal analyzer (AnalyzeTokens / stmtcheck). No action is required for typical usage. If you previously relied on textual error contents, switch to sentinel detection with:

	if errors.Is(err, libschema.ErrDataAndDDL) { ... }

### Practical Guidance

- Prefer one logical schema change per migration script. Split complex DDL + data backfills into (a) pure DDL then (b) a follow-up data migration.
- Always add IF EXISTS / IF NOT EXISTS when available; it improves safety for retry and makes intent explicit.
- For maintenance commands without guards (e.g. VACUUM), mark as non-transactional only if required by the database engine; otherwise keep transactional for atomic rollback.

### Future Extensions (Subject to Change)

- Additional dialect-specific normalization (e.g. handling edge-case index algorithm clauses)
- Configurable strictness for guard detection (currently simple textual pattern—false positives inside string literals are tolerated but harmless)
- Public exposure of classification results for higher-level tooling
