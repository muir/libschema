...
// TestDialectSpecificCoverage expands coverage for Postgres vs MySQL easy-fix distinctions
func TestDialectSpecificCoverage(t *testing.T) {
    type tc struct {
        name    string
        dialect Dialect
        sql     string
    ...
    for _, c := range cases {
        ...
        if len(stmts) == 0 {
            t.Fatalf("%s: no statements classified", c.name)
        }
        if agg != c.want {
            t.Errorf("%s: got flags 0x%x want 0x%x", c.name, agg, c.want)
        }
    }
}
...
func TestGuardInStringLiteral(t *testing.T) {
    ...
    if agg & IsNonIdempotent != 0 {
        t.Errorf("expected idempotent classification due to naive regex, got non-idempotent flags=0x%x", agg)
    }
}
