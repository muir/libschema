...
func ClassifyTokens(d Dialect, ts sqltoken.Tokens) (stmts []StatementFlags, aggregate uint32) {
    ...
    if len(split) > 1 { aggregate |= IsMultipleStatements }
    for _, s := range split {
        if len(s) == 0 { continue }
        txt := s.String()
...
func classifyCreate(d Dialect, txt string) uint32 {
    ...
    if d == DialectPostgres { listEasy = pgCreateEasy }
    var matchedEasy bool
    for _, p := range listEasy {
        if strings.HasPrefix(low, p) { matchedEasy = true; break }
    }
    if matchedEasy {
...
func classifyDrop(d Dialect, txt string) uint32 {
    f := IsDDL
    low := strings.ToLower(strings.TrimSpace(txt))
    if ifExistsRE.MatchString(low) { return f }
    f |= IsNonIdempotent
    listEasy := mysqlDropEasy
    if d == DialectPostgres { listEasy = pgDropEasy }
    for _, p := range listEasy {
        if strings.HasPrefix(low, p) { f |= IsEasilyIdempotentFix; break }
    }
    return f
}
