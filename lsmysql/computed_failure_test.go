...
// TestComputedFailure ensures a transactional computed MySQL migration records failure status.
func TestComputedFailure(t *testing.T) {
	dsn := os.Getenv("LIBSCHEMA_MYSQL_TEST_DSN")
	if dsn == "" { t.Skip("Set $LIBSCHEMA_MYSQL_TEST_DSN to test libschema/lsmysql") }
	...
	dbase, _, err := New(log, "test_mysql_fail", s, db)
	require.NoError(t, err)
